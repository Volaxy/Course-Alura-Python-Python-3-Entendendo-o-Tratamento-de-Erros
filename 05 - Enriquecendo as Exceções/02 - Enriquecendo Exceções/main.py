...
class CheckingAccount:
    ...
    def transfer(self, value, favored):
        ...
        if value > self.__balance:
            raise InsufficientFundsException("", self.balance, value, "abcd", "test", "other test")
    ...
    def withdraw(self, value):
        if value < 0:
            raise ValueError("The amount to be withdrawn cannot be less than 0")
        if value > self.__balance:
            # raise InsufficientFundsException(balance=self.__balance, value=value)
            # raise InsufficientFundsException("Insufficient balance to complete the transaction")
            raise InsufficientFundsException("", self.balance, value, "abcd", "test", "other test")
